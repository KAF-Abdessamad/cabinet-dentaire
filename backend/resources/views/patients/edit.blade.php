...
@section('title', 'Modifier Patient - ' . $patient->full_name)
...
<div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            ...
            <h1 class="text-3xl font-bold text-gray-900">Modifier Patient</h1>
            <p class="mt-2 text-gray-600">{{ $patient->full_name }}</p>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm">
            <form action="{{ route('patients.update', $patient) }}" method="POST" class="p-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="address" id="address" rows="3"
                                      class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500">{{ old('address', $patient->address) }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="cin" id="cin" value="{{ old('cin', $patient->cin) }}"
                                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500">
                        </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="allergies" id="allergies" rows="2" placeholder="Pénicilline, latex, etc..."
                                      class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500">{{ old('allergies', $patient->allergies) }}</textarea>
                        </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="medical_history" id="medical_history" rows="3" placeholder="Maladies chroniques, chirurgies, traitements en cours..."
                                      class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500">{{ old('medical_history', $patient->medical_history) }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-between pt-6 border-t border-gray-200">
                    <form action="{{ route('patients.destroy', $patient) }}" method="POST" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce patient ? Cette action est irréversible.');">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="px-6 py-3 bg-red-600 text-white font-medium rounded-lg hover:bg-red-700 transition-colors">
